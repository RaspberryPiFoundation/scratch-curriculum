...
# Step 2: Random balloon { .activity }
...
# Step 3: Popping the balloon { .activity }

Lets allow the player to pop the balloon!
...
+ A much better way of getting lots of balloons is to _clone_ the balloon sprite.
	...
	You can now attach the code you've just removed to a new `when I start as a clone` {.blockevents} event. You should also replace the `hide` {.blocklooks} block in the balloon-clicking script with a `delete this clone` {.blockcontrol} block.
