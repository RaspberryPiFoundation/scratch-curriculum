...
+ Используйте кнопку `выбрать спрайт из библиотеки` чтобы добавить новый спрайт приведения в проект (используйте костюм __фантастика/ghost1__ ). 

__Теперь мы хотим заставить наше приведение двигаться__
...
Переменная скорость будет контролировать, как быстро будет двигаться наше приведение. Мы используем переменную так, что мы можем контроллировать, как быстро движется приведение во время выполнения игры.

+ Мы хотим, чтобы приведение начало двигаться, когда стартует игра, __сделайте скрип, вроде этого__:
...
+ Чтобы остановить застревание приведения, мы должны дать возможность идти другим путем, когда оно касается края экрана. Измените ваш существующий скрипт, добавив к нему блок "если на краю, оттолкнуться" (раздел - движение).
...
+ __Попробуйте изменить переменную скорость и посмотрите, как движется приведение, быстро или медленно__
+ __Как вы можете сделать приведение летящим всё быстрее, в зависимости от того, как долго оно летает.?__
...
Чтобы превратить это в игру, мы должны дать игрокам сделать что-то. Они должны кликнуть по призраку, чтобы он исчез. Когда по приведению кликают, мы хотим, чтобы он исчез и издал звук.
...
Мы имеем приведение, но теперь мы хотим сделать игру! Мы хотим получать очки каждый раз, когда мы кликаем на приведение, но так же мы хотим ограничить лимит времени на игру. Мы можем использовать переменные для счета и таймера.

## Список действий { .check}

+ Создайте новую переменную для всех спрайтов, называемую __счет__, и затем скрипт для приведения, которые увеличивает эту переменную, когда по нему попадают.
...
Не забудьте победиться вашей игрой с друзьями и семьей, нажав кнопку __Поделиться__ в верхнем правом углу!
