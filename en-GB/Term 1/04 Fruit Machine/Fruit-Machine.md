...
2. Click *Events* and drag a `when flag clicked` { .blockbrown} into the scripts area. This will be triggered when we click the green flag.
...
5. Now click *Looks* and drag in a `next costume` { .blockpurple}
6. How do we slow it down so it isn’t changing so quickly? Click the *Control* tab and drag in a `wait 1 secs` { .blockyellow}
...
2. At the start of the game, the sprite won't have been clicked so we'll set the variable to be equal to "NO". 
  ...
  ```scratch
    ...
    forever 
        ...
        wait (0.1) secs
    ```
3. Now we'll set the variable **stopped** to the "YES" when someone clicks on the sprite.  

    ```scratch
        when this sprite clicked
        set [stopped v] to (YES)
    ```
4. Finally we need to make the sprite stop changing costume when the variable **stopped** changes to "YES". Add an `if...then` { .blockyellow} loop and use a new **equals** `[] = []` { .blockgreen} operator block (found under the *Operators* tab) to check if **stopped**  is still "NO".
...
1. If you look under the *costumes* tab for a sprite then you'll see that each costume has a number. You can specify
which costume a sprite is wearing using either its name or its number.
...
the costume number. 
3. We can also use exactly the same block in the **forever** loop so that the sprite switches to a different costume each time it changes during the game.
...
Change the difficulty of the game somehow. Just making the costumes change quicker is failry easy. Can you come up with something more imaginative. Some ideas you might like to try:
...
4. How can we detect when all the sprites have stopped? Remember we use the **stopped** variable to record whether each sprite has been clicked? Let's check the **stopped variable** 
for the Fruit3 sprite to see if the game is over.  Select the Fruit3 sprite and then can use a `x position of Fruit3` { .blockblue} block from the *Sensing* tab, but change **x position** to **stopped**.
...
5. To check to see that __all three__ fruit sprites have had their **stopped** variables set to YES,
we can use the `and` {. blockgreen} operator. This is a compicated block that can be quite fiddly to assemble, so try and put 
...
__The aim of the game is to click on the sprites so they stop while showing the same costume. It would be nice to also display a 
message that told you whether you'd won or lost.__

1. We wrote the code to check that the game was over in __step 6__, so all we need to do now is check to see if the player has won. Go back to the 
backdrops and and add some more text to the GameOver backdrop so that also displays the word "WIN". Then change its name to "Win".
2. Copy the backdrop again to create one with a "Lose" message. Give it the name "Lose".
3. Now we need some code to work out which backrop to display once the game is over. We can use an `if...then...else` { .blockyellow} block to see if the player has won or lost by comparing each **costume #**  (costume number) using
  a similar `x position of Sprite` { .blockblue} block like we did before. This time, instead of looking at the **stopped** variable, we can check the **costume #** and see if Fruit1 has the same costume as Fruit2, and if Fruit2 has the same costume as Fruit3.
 ...
 ## Test Your Project { .flag}
__Click the green flag.__ Does the correct message appear when the game has finished? What will happen if
each sprite's costume numbers don't match (for example, if Fruit2's costume number 3 is an apple and Fruit3's costume number 3 is 
a melon)?
...
You'll probably need to think about using a different way of starting the game each time it is played instead of the `when flag clicked`.{blockbrown} Then you can store values in variables that are remembered between each round of the game. 
